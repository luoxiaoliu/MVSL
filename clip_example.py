...
def get_args():
    ...
    parser.add_argument(
        '--image-path',
        type=str,
        default='/home/zrway/willow/pytorch-grad-cam-master/examples/dogs.png',
...
class ImageClassifier(nn.Module):
    ...
    def forward(self, x):#x[1, 3, 224, 224]
        ...
        probs = logits_per_image.softmax(dim=1)#[1, 5]
...
if __name__ == '__main__':
    ...
    input_tensor = preprocess_image(rgb_img, mean=[0.5, 0.5, 0.5],
                                    std=[0.5, 0.5, 0.5]).to(args.device)#[1, 3, 224, 224]
    ...
    grayscale_cam = cam(input_tensor=input_tensor,
                        ...
                        aug_smooth=args.aug_smooth)#(1, 224, 224)

    # Here grayscale_cam has only one image in the batch
    grayscale_cam = grayscale_cam[0, :]#(224, 224)

    cam_image = show_cam_on_image(rgb_img, grayscale_cam)
    cv2.imwrite(f'{args.method}_cam_3.jpg', cam_image)
