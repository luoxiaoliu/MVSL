...
class BTMRI(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        if num_shots >= 1:
            ...
            if os.path.exists(preprocessed):
                ...
                train = self.generate_fewshot_dataset(train, num_shots=num_shots)#16*4 = 64
                val = self.generate_fewshot_dataset(val, num_shots=min(num_shots, 4))#4*4=16
    ...
    def read_split(filepath, path_prefix):
        def _convert(items):
            out = []
            for impath, label, classname in items:  # train=2854
        ...
        split = read_json(filepath)  # len(split["train"])=2854;len(split["test"])=1717;len(split["val"])=1141
